...
| E) Google API Key hardcoded (iOS + Web), keine manuelle Hinterlegung | **Superseded 2026-05-17**: Key wird über `VITE_GOOGLE_MAPS_API_KEY` injiziert; keine harte Key-Konstante mehr in `src/config/googleMaps.js` oder `ios/App/App/AppDelegate.swift`. | Aktualisiert |
| F) Manuelle Key-Logik entfernen (UI/Settings/Env-Override/Storage) | Runtime-Setter/LocalStorage-Key-Pfade bleiben entfernt; Key-Eingabe-UI bleibt entfernt; produktiver Pfad läuft über Build-Environment. | Aktualisiert |
